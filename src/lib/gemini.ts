...
export interface VoiceTurnOutput {
    draftPatch: Partial<VoiceTurnInput['currentDraft']>;
...
export function getMissingRequiredFields(draft: VoiceTurnInput['currentDraft']) {
    const missing: string[] = [];
    if (!draft.description?.trim()) missing.push("description");
    if (!draft.category?.trim()) missing.push("category");
    if (!draft.locationText?.trim()) missing.push("locationText");
    return missing;
}
...
export async function generateVoiceSummary(complaints: any[], userName: string) {
    ...
    const tryGenerate = async (modelName: string) => {
        ...
        const prompt = `
        You are 'Delhi CM Grievance Voice Assistant', an AI helper for citizens of Delhi NCT.
